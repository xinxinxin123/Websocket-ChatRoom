...
$(document).ready(function(){
	$("#rec").click(function(){
		$.ajax({url:"/message/qryAllMessage",data:{staffId:staffId,targetId:targetId},
...
function show(number){
	...
	if(number==z && flag ==1){
		...
		for(var j = d-1;j>=0;j--){
			...
			}else if(recd[p+j].type == "N"){
                a.append("<div><img src="+ recd[p+j].headUrl + " class=\"img-circle\"><span class=\"label label-info\">"+recd[p+j].staffName+"</span>"+new Date(recd[p+j].createdDt).pattern("yyyy-MM-dd EEE hh:mm:ss")+"<div class=\"amessage\" id=\"singleRec"+j+"\"><pre class=\"prettyprint\"id=\"content"+j+"\"></pre></div><br></div>");
		...
		for(c in recd){
			...
			}else if(recd[p].type == "N" || recd[p].type == "U"){
                a.append("<div><img src="+ recd[p].headUrl + " class=\"img-circle\"><span class=\"label label-info\">"+recd[p].staffName+"</span>"+new Date(recd[p].createdDt).pattern("yyyy-MM-dd EEE hh:mm:ss")+"<div class=\"amessage\" id=\"singleRec"+p+"\"><pre class=\"prettyprint\"id=\"content"+p+"\"></pre></div><br></div>");
