...
function getFileList(){
	...
	$.ajax({
			type:"get",
			url:"/shareFile/getFileList",
			async:true,
			success:function(data){
				if (data) {
                    var res = "";
                    for(i in data ){
                        var res = res + "<pre class='prettyprint'>"+"文件名："+ data[i].foreName+ "<br>上传人："+ data[i].uploader +"<br>上传时间："+data[i].createDate+"<a href=\"download?fileId="+ data[i].id+  "\"target=\"view_window\">下载</a></pre>";
                    }
                    $("#fileList").append(res);
				}
			}
